...
async function SwitchRegion(play) {
	const Group = $.read('BiliArea_Policy') || '𝗕𝗶𝗹𝗶𝗯𝗶𝗹𝗶'; //Your blibli policy group name.
	const CN = $.read('BiliArea_CN') || 'DIRECT'; //Your China sub-policy name.
	const TW = $.read('BiliArea_TW') || '𝗧𝗪'; //Your Taiwan sub-policy name.
	const HK = $.read('BiliArea_HK') || '𝗛𝗞'; //Your HongKong sub-policy name.
...
function SwitchStatus(status, original, newPolicy) {
	if (status && typeof original !== 'number') {
		return `『${original}』 𝑠𝑤𝑡𝑖𝑐ℎ𝑒𝑑 𝑡𝑜 『${newPolicy}』`;
	} else if (original === 2) {
